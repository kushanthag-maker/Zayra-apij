...
const cinesubz = require('./cinesubz'); // Cinesubz R2 signed download API
...
const CATALOG = [
  ...
  { method: 'GET', path: '/api/v1/cinesubz/list', group: 'Cinesubz', title: 'Cinesubz Content List', desc: 'List available Cinesubz / R2 content ids (title, filename, contentType). Free (no credit). Requires R2 env vars + content.json.', params: [] },
  { method: 'GET', path: '/api/v1/cinesubz/download', group: 'Cinesubz', title: 'Cinesubz Signed Download', desc: 'Generate an expiring signed download URL from private Cloudflare R2. Use only for files you own or are licensed to distribute. expires = 60–3600 seconds (default 900).', params: [{ name: 'id', example: 'sample-video', required: true }, { name: 'expires', example: '900', required: false }] },
];
