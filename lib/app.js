...
const cinesubz = require('./cinesubz'); // Cinesubz downloads (direct URL or optional R2)
...
const CATALOG = [
  ...
  { method: 'GET', path: '/api/v1/cinesubz/list', group: 'Cinesubz', title: 'Cinesubz Content List', desc: 'List available download content (title, filename, contentType). Free (no credit). No env vars required — uses content.json.', params: [] },
  { method: 'GET', path: '/api/v1/cinesubz/download', group: 'Cinesubz', title: 'Cinesubz Download', desc: 'Get a download URL for a content id from content.json. Works with public "url" (no env needed) or optional R2 "key" + S3 env for signed private links. expires only applies to R2 mode (60–3600s, default 900).', params: [{ name: 'id', example: 'sample-video', required: true }, { name: 'expires', example: '900', required: false }] },
];
...
async function runPublic(route, req, q, body, user) {
  ...
  switch (r) {
    ...
    // ---- Cinesubz (R2 signed downloads) ----
    case 'cinesubz/list': return cinesubz.list();
    case 'cinesubz/download': return cinesubz.download(q);
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (route.startsWith('v1/')) {
      ...
      if (status < 400 && route !== 'v1/files/list' && route !== 'v1/cinesubz/list') { u.credits -= 1; } // listing is free
